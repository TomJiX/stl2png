...
let zoffset = 0;
let laser_max_pow = 20;
let outputfilename = "output.png";
let coeff = [-0.97499805 , 7.17054653];
coeff.reverse();
...
{	
    ...
    if (global_maxY < 0.1)
        alert(global_maxY);
    global_scale_factor = newsize / global_maxX;
    global_scale_factor = Math.min(global_scale_factor, newsize / global_maxY);
    
    
    for (let i = 0; i < len; i++) {
        t = triangles[i];
        t.vertex[0][0] *= global_scale_factor;
        t.vertex[0][1] *= global_scale_factor;
        t.vertex[0][2] *= global_scale_factor;

        t.vertex[1][0] *= global_scale_factor;
        t.vertex[1][1] *= global_scale_factor;
        t.vertex[1][2] *= global_scale_factor;

        t.vertex[2][0] *= global_scale_factor;
        t.vertex[2][1] *= global_scale_factor;
        t.vertex[2][2] *= global_scale_factor;
    ...
    global_maxX *= global_scale_factor;    
    global_maxY *= global_scale_factor;   
    global_maxZ *= global_scale_factor;    
}
...
function correction(height,laser_max_pow)
{
    //console.log(coeff);  
    let x_pow = Array(coeff.length).fill((global_maxZ-height)/global_scale_factor);
    const powers = x_pow.map((num, i) => coeff[i]*(num ** (i+1) ));

    let laser_pow=Math.max(powers.reduce((a, b) => a + b, 0),0);
    
    let corrected_pixel=255-(laser_pow/laser_max_pow)*255;
...
function  calculate_line(data, Y, elem, context, pixels) 
{
    ...
    for (let X = 0; X < global_maxX; X++) {
            let d = (get_height(X, Y) - zadd)             
            let c = correction(d,laser_max_pow);
            data[(X + Y * global_maxX) * 4 + 0 ] = c;
            data[(X + Y * global_maxX) * 4 + 1 ] = c;
            data[(X + Y * global_maxX) * 4 + 2 ] = c;
            data[(X + Y * global_maxX) * 4 + 3 ] = c;
    }
...
function set_F_coeff(val1,val2)
{
    let news1 = Math.floor(parseFloat(val1));
    console.log("Setting coeff 1 to " + val1);
    let news2 = Math.floor(parseFloat(val2));
    console.log("Setting coeff 2 to " + val2);
    coeff = [news1,news2];  
    coeff.reverse();
}
...
function load(evt) 
{
    ...
    if (evt.target.readyState == FileReader.DONE) {
        ...
        console.log("maxZ ="+global_maxZ/global_scale_factor+"mm");
